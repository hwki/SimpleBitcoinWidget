...
public class NewCurrenciesTest extends TestCase {
    ...
    public void testProviders() throws InterruptedException {
        ...
        for (EthereumExchange eth : values) {
            ...
            Set<String> existingCurrencies = new HashSet<>(Arrays.asList(resources.getStringArray(eth.getCurrencies())));
            for (String currency : currencies) {
                try {
                    String value = eth.getValue(currency);
            ...
            if (!added.isEmpty()) {
                ...
                addedMap.put(eth.getLabel(), TextUtils.join(",", added));
            }
            if (!removed.isEmpty()) {
                Collections.sort(removed);
                removedMap.put(eth.getLabel(), TextUtils.join(",", removed));
            }
        }
